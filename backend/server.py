...
async def mobile_dashboard(user=Depends(require_staff)):
    ...
    overdue = await db.leads.find(
        {'next_follow_up_date': {'$lt': today, '$exists': True, '$nin': [None, '']},
         'status': {'$nin': ['Joined', 'Lost', 'No Response']}},
        {'_id': 0, 'id': 1, 'first_name': 1, 'last_name': 1, 'phone': 1, 'email': 1,
...
def _review_email_html(name: str, review_url: str) -> str:
    safe_name = escape_html(name)
    safe_url = escape_html(review_url)
    return f"""<!DOCTYPE html><html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#F7F5F0;font-family:'Helvetica Neue',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px;">
  <tr><td align="center">
    <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
      <tr><td style="background:#0D5D3E;padding:28px 36px;text-align:center;">
        ...
        <p style="margin:8px 0 0;color:#ffffff;font-size:24px;font-weight:800;">Welcome to the family, {safe_name}!</p>
      </td></tr>
      <tr><td style="padding:36px;text-align:center;">
        <p style="margin:0 0 8px;font-size:32px;">&#127947;</p>
        <p style="margin:0 0 16px;font-size:17px;font-weight:700;color:#1a1a1a;">You are officially part of the crew.</p>
        <p style="margin:0 0 28px;font-size:14px;color:#555;line-height:1.65;">
          Stoked to have you at Santa Cruz Strength. We would love to hear how your first experience has been. It only takes 10 seconds.
        </p>
        <table cellpadding="0" cellspacing="0" style="margin:0 auto 24px;">
          <tr><td style="background:#0D5D3E;border-radius:10px;">
            <a href="{safe_url}" style="display:inline-block;padding:14px 32px;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;letter-spacing:0.3px;">
      ...
      <tr><td style="border-top:1px solid #eee;padding:16px 36px;background:#fafaf9;text-align:center;">
        <p style="margin:0;font-size:11px;color:#aaa;">Santa Cruz Strength &middot; 151 Harvey West Blvd Ste D, Santa Cruz CA</p>
      </td></tr>
    </table>
  </td></tr>
</table></body></html>"""
...
<p>The one you finish. A 45 minute session three times per week that you actually complete will produce far better results over a year than an aspirational two hour program you abandon after three weeks. For most people with jobs and families, the ideal workout is the one that gets done consistently.</p>''',
    ...
    logger.info(f'[SEED] Upserted {len(posts)} blog posts (new slugs inserted, existing untouched)')
...
async def startup():
    ...
    seed_content = [
        ...
        # Home Page
        {'key': 'home_hero_headline', 'value': 'REAL STRENGTH.\nREAL PEOPLE.'},
        {'key': 'home_hero_subtitle', 'value': 'Strength looks different for everyone.'},
        ...
        {'key': 'home_hero_subtext', 'value': 'You don\u2019t have to be a \u201cserious lifter\u201d to belong here. No matter your goals, you belong at Santa Cruz Strength. '},
        {'key': 'home_benefits_headline', 'value': 'STRENGTH FOR EVERYONE.'},
        {'key': 'home_benefits_subtitle', 'value': 'Santa Cruz Strength is a community-centered gym where you can build strength, move your body, and feel more at home in yourself. '},
        {'key': 'home_environment_headline', 'value': 'WHAT TRAINING\nHERE FEELS LIKE'},
        {'key': 'home_environment_text', 'value': 'Most people come to build strength and stay because of how it feels to be here. We\u2019ve built a space that prioritizes respect, focus without pressure, and a sense of belonging. '},
        {'key': 'home_environment_subtext', 'value': 'You can train hard here. You can also take your time. All are welcome.'},
        {'key': 'home_who_headline', 'value': 'IF YOU HAVE A BODY,\nYOU BELONG HERE.'},
        {'key': 'home_who_text', 'value': 'There\u2019s no single type of person who joins Santa Cruz Strength, just the common thread of resistance training.'},
        {'key': 'home_who_subtext', 'value': 'We are a queer-friendly space, and we are committed to creating an environment where all identities are respected.'},
        # Training Page
    ...
    if ALLOW_SEEDING and team_count == 0:
        seed_team = [
            # Production roster: 9 members, exact names / roles / photo_urls / sort orders.
            # Photo URLs are the live Emergent media URLs. Members without a
            # photograph get an empty string and the frontend renders initials.
            ...
            {'id': str(uuid.uuid4()), 'name': 'Syon', 'role': 'Strength & Conditioning Coach Extraordinaire', 'bio': '', 'photo_url': 'https://santa-cruz-dev.emergent.host/api/media/1dab09dd-5e3c-4649-9617-3cc274037829', 'category': 'trainer', 'sort_order': 0, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
            {'id': str(uuid.uuid4()), 'name': 'Morghan ', 'role': 'Resident Olympian', 'bio': '', 'photo_url': '', 'category': 'trainer', 'sort_order': 1, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
            {'id': str(uuid.uuid4()), 'name': 'Lexi', 'role': 'Strength Coach & Acrobatic Enthusiast', 'bio': 'Lexi is a Certified Strength and Conditioning Coach through the NSCA and holds a BS in Kinesiology. With over 7 years of personal training experience and 6 years as a competitive athlete herself, she brings a deep understanding of movement, strength, and injury prevention. Whether your goal is to get stronger, faster, leaner, or simply to move better, she is here to guide you.', 'photo_url': 'https://customer-assets.emergentagent.com/job_da9a4a5d-572a-4858-ad1e-163b4849fc8c/artifacts/457i1107_Lexi.jpg', 'category': 'trainer', 'sort_order': 2, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
            {'id': str(uuid.uuid4()), 'name': 'Chris', 'role': 'Strength Coach & Powerlifting Powerhouse', 'bio': '', 'photo_url': 'https://santa-cruz-dev.emergent.host/api/media/e362853e-89b5-4b71-b85d-f25d69676297', 'category': 'trainer', 'sort_order': 3, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
            {'id': str(uuid.uuid4()), 'name': 'Dali', 'role': 'Grip Strength Specialist', 'bio': '', 'photo_url': '', 'category': 'trainer', 'sort_order': 4, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
            {'id': str(uuid.uuid4()), 'name': 'Kat', 'role': 'Strength Coach', 'bio': '', 'photo_url': '', 'category': 'trainer', 'sort_order': 5, 'is_visible': True, 'created_at': now_utc().isoformat(), 'updated_at': now_utc().isoformat()},
        ]
    ...
    # Seed blog posts — upsert by slug so partial fills get completed
    if ALLOW_SEEDING:
