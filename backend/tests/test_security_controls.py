...
class SecurityControlTests(unittest.TestCase):
    def test_cors_accepts_platform_wildcard_and_rejects_invalid_values(self):
        # Platform-managed wildcard is accepted (L-5 deployment learning)
        self.assertEqual(parse_cors_origins('*', 'production'), ['*'])
        self.assertEqual(parse_cors_origins('*', 'development'), ['*'])
        # Invalid values still rejected
        for value in ('https://example.com/path', 'https://user:pass@example.com'):
