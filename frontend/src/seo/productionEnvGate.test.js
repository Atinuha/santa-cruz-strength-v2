...
function validate(backendUrl, prerenderUrl) {
  // Both REACT_APP_BACKEND_URL and PRERENDER_API_URL are required.
  // When prerenderUrl is not specified, default it to the same as backendUrl
  // so existing single-argument tests exercise the matched-origin path.
  const prerender = prerenderUrl !== undefined ? prerenderUrl : backendUrl;
  return spawnSync(process.execPath, [script], {
    env: { ...process.env, REACT_APP_BACKEND_URL: backendUrl, PRERENDER_API_URL: prerender },
    encoding: 'utf8',
  });
}
...
describe('production backend URL gate', () => {
  ...
  test('accepts matching alternate origin', () => {
    const result = validate('https://crm-staff-portal-1.emergent.host');
    expect(result.status).toBe(0);
  });
  ...
  test('rejects mismatched origins', () => {
    const result = validate(
      'https://api.santacruzstrength.com',
      'https://other.santacruzstrength.com',
    );
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('same production origin');
  });

  test('rejects preview PRERENDER_API_URL', () => {
    const result = validate(
      'https://api.santacruzstrength.com',
  ...
  test('rejects empty PRERENDER_API_URL', () => {
    const result = validate('https://api.santacruzstrength.com', '');
    expect(result.status).not.toBe(0);
  });
});
