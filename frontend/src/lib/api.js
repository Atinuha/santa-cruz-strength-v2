...
export const login = (email, password, deviceToken = null) =>
  api.post('/auth/login', { email, password, ...(deviceToken ? { device_token: deviceToken } : {}) });
export const verifyOtp = (email, otp, rememberDevice = false) =>
  api.post('/auth/verify-otp', { email, otp, remember_device: rememberDevice });
