...
// Drift check is scoped to slugs that have a registered route. Articles in the
// backend source that are not yet deployed (no route in the registry) are not
// checked — they will be validated when their routes are added.
const registeredBlogSlugs = new Set(
  registry.routes
    .filter((route) => route.path.startsWith('/blog/') && route.path !== '/blog')
    .map((route) => route.path.slice('/blog/'.length))
);
const blogFaqDrift = [...new Set([...Object.keys(blogFaq), ...Object.keys(renderedBlogFaq)])]
  .filter((slug) => registeredBlogSlugs.has(slug))
